...
--
-->>
-- After doing 'require "cleft_lua_ffi"' from a script, you get access to the
-- cleft_lua_ffi function namespace which includes a curated and safened set of
...
--Example Useage:
-->>
-- require "cleft_lua_ffi"
--
-- print("A")
-- if not cleft_lua_ffi.CreateDirectory(GetInfo(66).."test\\test", true) then
--    print("Failed A")
-- end
--
-- print("B")
...
BOOL __stdcall CopyFileA(LPCTSTR lpExistingFileName, LPCTSTR lpNewFileName, BOOL bFailIfExists);
...
local function CopyFile(src, dest, acceptable_errors, retry_count)
   acceptable_errors = acceptable_errors or {}
   ...
   local bail_count = 0
   local function TryNames()
      local ret = ffi.C.CopyFileA(src, dest, true)
      ...
      if err_no == 183 then
	 ColourNote("yellow", "", "Trying again with " .. retry_count - bail_count .. " retries left.")
	 return TryNames()
      else
	 return 0
      end
